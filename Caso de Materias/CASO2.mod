...
s.t. requicitos1 {l in J, i in I}: (card(REQ[i]))*w[i,l]<=sum{m in REQ[i],j in J:j<l}x[m,j]; #forzar 1
s.t. requicitos0 {l in J, i in I}: 1000*w[i,l]+(card(REQ[i])-1)*(1-w[i,l])>=sum{m in REQ[i],j in J:j<l}x[m,j]; #forzar 0 
...
#máximo 20 o 21 créditos por semestre
s.t. res3{j in J}: x[20,j]<=z[j];#Etica
s.t. res4{j in J}: x[15,j]<=z[j];#Fe
