...
/*Restricciones*/
s.t. Demandaatendida: sum{j in J} d[j] <=sum{i in I,j in J}x[i,j]*d[j];
s.t. Porcentajeminimo{i in I}: sum{j in J} d[j]*x[i,j] >=0.9*cap[i]*y[i]; 
s.t. Porcentajemaximo {i in I}: sum{j in J} d[j]*x[i,j] <= cap[i]*y[i];
s.t. Antencionclientes {j in J}: sum{i in I} x[i,j]=1;
s.t. Minclientes {i in I}: sum{j in J} x[i,j] >= 3*y[i];
